...
When mounted the diagram edtior will need to convert the exisitng workflow YAML into nodes in a diagram. This functionality is done with the `importFromWorkflowData` function in [src/components/diagram-editor/import.js](https://github.com/direktiv/direktiv-ui/blob/main/src/components/diagram-editor/import.js). 
...
Nodes are linked to their JSON schema by their schemaKey property. The StateNoop Node the schemaKey is `stateSchemaNoop`:
```js
...
        data: {
            schemaKey: 'stateSchemaNoop',

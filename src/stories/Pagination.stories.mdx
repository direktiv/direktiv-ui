...
The state of `pageHandler.pageParams` from the previous example can be manipulated using the Pagination component. Simply passed it to the pageHandler prop and the Pagination component will automatically `pageParams` when the user changes pages.
...
For example: If we take the example above: If the user clicked the next page button to go to page 2 on the Pagination component `pageHandler` will updates its `pageParams` property to equal `limit=10&offset=10`. This change to `pageParams` is what ultimately changes the `data` being returned from the `useNodes` direktiv-react-hook.
